...
class RankingLoss(nn.Module):
    ...
    def semi_hard_negative(self, loss):
        negative_index = np.where(np.logical_and(loss < self.config.margin, loss > 0))[0]
